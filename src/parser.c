...
void save_mAP(list *list_mAP, char* filename) {
    ...
    for(int i=0; i<list_mAP->size; i++) {
        ...
        fwrite(&iterBatch, sizeof(int), 1, fp);
        fwrite(&TP, sizeof(int), 1, fp);
        fwrite(&TN, sizeof(int), 1, fp);
        fwrite(&FP, sizeof(int), 1, fp);
        fwrite(&FN, sizeof(int), 1, fp);
        fwrite(&precision, sizeof(float), 1, fp);
        fwrite(&recall, sizeof(float), 1, fp);
...
void save_loss(list *list_loss, char* filename) {
    ...
    for(int i=0; i<list_loss->size; i++) {
        _lossAcc *loss = (_lossAcc*)list_pop(list_loss);
        
        int iter = loss->iterBatch;
        float avg_loss = loss->avgLoss;
        float max_img_loss = loss->maxImgLoss;
        
        printf("%d %.5f %.5f\n",loss->iterBatch,loss->avgLoss,loss->maxImgLoss);
